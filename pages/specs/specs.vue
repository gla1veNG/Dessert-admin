<template>
	...
	<view class="attribute gener" v-for="(item,index) in sku_data.sku" :key="index">
		...
		<view class="edit entry" v-if="item.att_data.length > 0" v-for="(item_add,index_add) in item.att_data"
			...
			<input type="text" v-model="item_add.add_val" :placeholder=" '请输入' + item_add.att_name "
				placeholder-class="I-style" cursor-spacing="50" />
		</view>
		<view class="edit entry">
			<text>价格</text>
			<input type="text" v-model="item.price" placeholder="请输入价格" placeholder-class="I-style"
				cursor-spacing="50" />
			<text>￥</text>
		</view>
		<view class="edit entry">
			<text>库存</text>
			<input type="text" v-model="item.stock" placeholder="请输入库存" placeholder-class="I-style"
				cursor-spacing="50" />
			<text>件</text>
		</view>
		<!-- 上传图片 -->
		<view class="specs-image">
			...
			<image class="delete-img" src="/static/detail/shanchu.svg" mode="widthFix" v-if="item.image != ''"
